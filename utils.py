...
def get_multi_street(center_lat, center_long, headings, pitch, fov, api_key, STREET_URL, SAT_URL):
  images=[]

  for heading in headings:
    street_params = get_street_view_params(center_lat, center_long, heading, api_key, pitch, fov)
    resp = requests.get(STREET_URL, params=street_params)
    img = Image.open(BytesIO(resp.content))
    images.append(img)
    #print(heading)
