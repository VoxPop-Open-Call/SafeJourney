...
def get_utm(row, utm_info: dict):
    res = utm.from_latlon(float(row[utm_info['lat_row']]), float(row[utm_info['long_row']]), utm_info['zone_paramter_1'], utm_info['zone_parameter_2'])

    return res[0], res[1]

def get_utm_array(lat, longt, utm_info: dict):
    res = utm.from_latlon(lat, longt, utm_info['zone_paramter_1'], utm_info['zone_parameter_2'])
...
def filter_grid(grid, address):
    ...
    area_of_interest = gpd.GeoDataFrame(index=[0], crs="EPSG:20791", geometry=[alpha_shape])

    grid['geos'] = [Point(x,y) for x,y in zip(grid['center_lat'], grid['center_long'])]
    ...
    df = pd.merge(grid, points_in_area_of_interest, left_index=True, right_on='index_right')
...
def get_drive_map_intersections(address, utm_info):
    ...
    intersections[['x_utm', 'y_utm']] = intersections.apply(lambda x: get_utm(x, utm_info), axis=1, result_type='expand')
...
def define_intersections(address, points_df, utm_info):
    ...
    points_df['geos_utm'] = [Point(row['x_utm'], row['y_utm']) for id, row in points_df.iterrows()]
    intersections['geos_utm'] = [Point(row['x_utm'], row['y_utm']) for id, row in intersections.iterrows()]
    ...
    found_inters = gpd.sjoin_nearest(intersection_gdf, data_lean_gdf, distance_col='distances')
...
def get_drive_map_midpoints_elevations(address, utm_info, url):
    ...
    midpoints = gpd.GeoDataFrame(data=midpoint_geos).drop_duplicates().reset_index().rename(columns={0:'geo'}).drop(columns='index')
    ...
    midpoints['elevations'] = [get_point_elevation(geo.y, geo.x, url) for geo in tqdm(midpoints['geo'])]
    midpoints[['x_utm', 'y_utm']] = midpoints.apply(lambda x: get_utm(x, utm_info), axis=1, result_type='expand')

    return midpoints

def get_variance_rank(address, points_df, utm_info, url):

    midpoints = get_drive_map_midpoints_elevations(address=address, utm_info=utm_info, url=url)

    points = midpoints[['x_utm', 'y_utm']].values
    tree = cKDTree(points)

    points_df[['x_utm', 'y_utm']] = points_df.apply(lambda x: get_utm(x, utm_info), axis=1, result_type='expand')
    ...
    nns = midpoints.iloc[transposed_idxs[:, 1]]#[aggregator_cols] # [['ad_id', 'ad_unitprice', 'ad_typology', 'ad_operation', 'date_ranges']]

    ret = pd.concat([
        nns_lisbon_ids.reset_index(drop=True),
        nns.reset_index(drop=True)
    ], axis=1)

    ret.columns = ['x_utm_grid', 'y_utm_grid', 'geo', 'lat', 'long', 'elevations', 'x_utm', 'y_utm']
    std_elevation_grouped = ret.groupby(['x_utm_grid', 'y_utm_grid'])['elevations'].std()
    data_per_loc_merge = points_df.merge(std_elevation_grouped, left_on=['x_utm', 'y_utm'], right_on=['x_utm_grid', 'y_utm_grid'])
    data_per_loc_merge['elevation_rank'] = pd.qcut(data_per_loc_merge['elevations'], 100, labels=np.arange(0,99), duplicates='drop')
...
def get_dist_to_cycle_lane(geojson_path, points_df, utm_info):
    ...
    points_df['geo'] = [Point(row['x_utm'], row['y_utm']) for idx, row in points_df.iterrows()]
    ...
    cycle_point_gdf[['x_utm', 'y_utm']] = cycle_point_gdf.apply(lambda x: get_utm(x, utm_info), axis=1, result_type='expand')
    cycle_point_gdf['geometry'] = [Point(point['x_utm'], point['y_utm']) for idx, point in cycle_point_gdf.iterrows()]
...
def grid_maker(boundaries: dict, overlap_factor: float = 1, ratio: float = 500 / 110500):
...
def create_community_score_from_pois(grid, df_pois, radius_in_meters):
    ...
    within_radius = tree_grid.sparse_distance_matrix(tree, max_distance=radius_in_meters)
    ...
    for idx, neighbour in tqdm(enumerate(neighbours)):
        ...
        if neighbour:
            ...
            neighbourhood_scores = np.sum([df_pois['accessibility_score'].iloc[x] for x in neighbour] * dist_matrix) / np.sum(dist_matrix)

            comscore_grid['neighbourhood_accessibility'].iloc[idx] = neighbourhood_scores
...
def address_scores_from_grid(addresses, grid):
    ...
    for address in addresses:
        ...
        scores = [grid['neighbourhood_accessibility'].iloc[x] for x in points_in_area_of_interest['index_right']]
