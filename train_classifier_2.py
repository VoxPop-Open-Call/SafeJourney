...
from model_utils import build_model, DataGenDepth, get_config_yaml, focal_loss, reset_seeds
...
PARSER = argparse.ArgumentParser(description="Script to train image classification model")
...
gss = GroupShuffleSplit(n_splits=1, test_size=test_size, train_size=train_size, random_state=42)
...
model.compile(optimizer=Adam(learning_rate=training_parameters["learning_rate"]),
                  loss=task_losses,
...
image_path = ''
train_datagen = DataGenDepth(train, relative_path=image_path, task_classes=task_classes,batch_size=training_parameters["batch_size"], image_path_column="path", augmentation=True)
val_datagen = DataGenDepth(test, relative_path=image_path, task_classes=task_classes, batch_size=training_parameters["batch_size"], image_path_column="path", shuffle=True)
...
history_path = os.path.join(
    model_output,
    f"model_{model_timestamp}_history")
